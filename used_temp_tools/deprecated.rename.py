            ...
            if filename.lower().endswith(".md"):
                ...
                try:
                    ...
                    try:
                        ...
                        original_basename_clean = sanitize_filename_part(
                            original_basename
                        )  # 也清理一下原始名字

                        # 构造新文件名
                        new_filename_base = (
                            f"{primary_clean}-{detail_clean}-{level_clean}"
                        )
                        if original_basename_clean:  # 如果原始文件名清理后不为空
                            new_filename = (
                                f"{new_filename_base}[{original_basename_clean}].md"
                            )
                        else:  # 如果原始文件名清理后为空（虽然不太可能）
                            new_filename = f"{new_filename_base}.md"

                        print(
                            f"  提取维度: primary='{primary}', detail='{detail}', level='{level}'"
                        )
                    ...
                    print(
                        f"  [成功] 文件已处理并保存到: {os.path.join(TARGET_DIR_NAME, new_filename)}"
                    )
                    processed_count += 1

                except FileNotFoundError:
                    print(
                        f"  [错误] 文件未找到（可能在处理过程中被删除）: {original_filepath}"
                    )
                    error_count += 1
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(SOURCE_DIR):
        print(
            f"错误：源目录 '{SOURCE_DIR_NAME}' 不存在于 {BASE_DIR}。请确保文件夹存在。"
        )
    else:
